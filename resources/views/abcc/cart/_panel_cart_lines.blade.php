        ...
        @if ($cart->cartlines->count())
            <table id="cart_lines" class="table table-hover">
                ...
                @foreach ($cart->cartlines as $line)
                    <tr>
                    <!-- td>{{ $line->id }}</td -->
                        <td title="{{ $line->product->id }}">
                            @if ($line->product->product_type == 'combinable')
                            ...
                            @if( \App\Configuration::get( 'ABCC_STOCK_SHOW' ) != 'none')
                                @if( \App\Configuration::get( 'ABCC_STOCK_SHOW' ) == 'amount')
                                    <div class="progress progress-striped">
                                        <div class="progress-bar progress-bar-{{ $line->product->stock_badge }}"
                                             title="{{ l('stock.badge.'.$line->product->stock_badge, 'abcc/layouts') }}" style="width: 100%">
                                            <span class="badge" style="color: #333333; background-color: #ffffff;">
                                                {{ $line->product->as_quantity('quantity_onhand') }}
                                            </span>
                                        </div>
                                    </div>
                                @else
                            ...
                            <div xclass="form-group">
                                <div class="input-group" style="width: 81px">
                                    ...
                                    <input class="input-line-quantity form-control input-sm col-xs-2" data-id="{{$line->id}}"
                                           ...
                                           value="{{ (int) $line->quantity }}" onclick="this.select()">

                                    <span class="input-group-btn">
                                      <button class="update-line-quantity btn btn-sm btn-lightblue" data-id="{{$line->id}}"
                                              data-quantity="{{ (int) $line->quantity }}"
                                              type="button" title="{{l('Apply', [], 'layouts')}}"
                        ...
                        <td class="text-right button-pad">
                            ...
                            <a class="btn btn-sm btn-danger delete-cart-line" data-id="{{$line->id}}" title="{{l('Delete', [], 'layouts')}}"
                               data-content="{{l('You are going to delete a record. Are you sure?', [], 'layouts')}}"
                               data-title="{{ '('.$line->id.') ['.$line->reference.'] '.$line->name }}" onclick="return false;"><i
