...
{{ $customer_rules->appends( Request::all() )->render() }}
        ...
        <ul class="pagination">
            <li class="active"><span style="color:#333333;">{{l('Found :nbr record(s)', [ 'nbr' => $customer_rules->total() ], 'layouts')}} </span>
            </li>
        </ul>
        <ul class="pagination" style="float:right; display: none">
            <li xclass="active" style="float:right;"><span style="color:#333333;border-color:#ffffff"> <div class="input-group"><span
                                class="input-group-addon" style="border: 0;background-color: #ffffff"
                                title="{{l('Items per page', 'layouts')}}">{{l('Per page', 'layouts')}}</span><input id="items_per_page_pricerules"
                                                                                                                     name="items_per_page_pricerules"
                                                                                                                     class="form-control input-sm items_per_page_pricerules"
                                                                                                                     style="width: 50px !important;"
                                                                                                                     type="text"
                                                                                                                     value="{{ $items_per_page_pricerules }}"
                                                                                                                     onclick="this.select()">
    <span class="input-group-btn">
      <button class="btn btn-info btn-sm" type="button" title="{{l('Refresh', 'layouts')}}" onclick="getCustomerPriceRules(); return false;"><i
